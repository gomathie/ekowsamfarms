        ...
        <div className="bg-white rounded-3xl border border-slate-200 shadow-lg overflow-hidden grid grid-cols-1 lg:grid-cols-12">
          <div className="lg:col-span-6 relative h-72 lg:h-auto min-h-[380px]">
            ...
            <div className="absolute bottom-6 left-6 right-6 text-white space-y-1">
              <span className="bg-brand-600 text-white font-extrabold text-[11px] px-2.5 py-1 rounded">
                CAPACITY: {activeDiv.capacity}
              </span>
              <h3 className="text-2xl font-black font-serif mt-2">{activeDiv.title}</h3>
              <p className="text-xs text-brand-300 font-semibold">{activeDiv.subtitle}</p>
            </div>
          </div>

          <div className="lg:col-span-6 p-8 space-y-6 flex flex-col justify-between">
            <div className="space-y-4">
              <div className="inline-flex items-center gap-2 bg-brand-50 text-brand-900 border border-brand-200 px-3 py-1 rounded-full text-xs font-bold">
              ...
              <div className="pt-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {activeDiv.keyProducts.map((prod, i) => (
                    <span key={i} className="bg-brand-100 text-brand-900 text-xs font-bold px-3 py-1 rounded-lg border border-brand-200">
                      {prod}
                    </span>
                  ))}
                </div>
            ...
            <div className="pt-4 border-t border-slate-100 flex flex-wrap items-center gap-3">
              <button
                ...
                className="bg-brand-600 hover:bg-brand-700 text-white font-bold px-5 py-2.5 rounded-xl text-xs transition-colors shadow-xs flex items-center gap-1.5"
              >
