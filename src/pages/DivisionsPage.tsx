        ...
        <div className="bg-white rounded-3xl border border-slate-200 shadow-lg overflow-hidden grid grid-cols-1 lg:grid-cols-12">
          <div className="lg:col-span-6 relative h-72 lg:h-auto min-h-[380px]">
            ...
            <div className="absolute bottom-6 left-6 right-6 text-white space-y-1">
              <span className="bg-emerald-600 text-white font-extrabold text-[11px] px-2.5 py-1 rounded">
                CAPACITY: {activeDiv.capacity}
              </span>
              <h3 className="text-2xl font-black font-serif mt-2">{activeDiv.title}</h3>
              <p className="text-xs text-emerald-300 font-semibold">{activeDiv.subtitle}</p>
            </div>
          </div>
              ...
              <div className="pt-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {activeDiv.keyProducts.map((prod, i) => (
                    <span key={i} className="bg-emerald-100 text-emerald-900 text-xs font-bold px-3 py-1 rounded-lg border border-emerald-200">
                      {prod}
                    </span>
                  ))}
                </div>
