    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/80 backdrop-blur-xs flex items-center justify-center p-4 font-sans">
      <div className="bg-white rounded-3xl shadow-2xl max-w-2xl w-full overflow-hidden border border-slate-100 relative my-8 animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="bg-emerald-950 text-white p-6 relative">
          <button
            onClick={onClose}
            className="absolute right-5 top-5 p-2 rounded-full bg-emerald-900 hover:bg-emerald-800 text-emerald-200 transition-colors"
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="space-y-4 bg-slate-50 p-4 rounded-2xl border border-slate-200 text-xs">
            <div>
              <div className="flex justify-between items-center mb-1.5">
                ...
                <span className="font-extrabold text-emerald-800 text-sm bg-emerald-100 px-3 py-0.5 rounded-full border border-emerald-200">
            ...
            {poultryType === 'layer' ? (
              <div>
                ...
                <input
                  ...
                  className="w-full p-2.5 rounded-xl border border-slate-300 bg-white font-bold text-emerald-900"
                />
              </div>
            ) : (
              <div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center">
                <div className="p-3 bg-emerald-50 rounded-xl border border-emerald-200">
                  ...
                  <span className="text-lg font-black text-emerald-950">{dailyEggCount.toLocaleString()}</span>
                  <span className="text-[10px] text-emerald-700 block">~88% lay rate</span>
                </div>

                <div className="p-3 bg-emerald-50 rounded-xl border border-emerald-200">
                  <span className="text-xs text-slate-500 font-medium block">Daily Crates</span>
                  <span className="text-lg font-black text-emerald-950">{dailyCrates}</span>
                  <span className="text-[10px] text-emerald-700 block">(30 eggs/crate)</span>
                </div>

                <div className="p-3 bg-emerald-50 rounded-xl border border-emerald-200">
                  <span className="text-xs text-slate-500 font-medium block">Monthly Feed</span>
                  <span className="text-lg font-black text-emerald-950">{layerFeedBagsPerMonth}</span>
                  <span className="text-[10px] text-emerald-700 block">50kg Mash Bags</span>
                </div>
              ...
              <div className="p-4 bg-emerald-900 text-white rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-3 shadow-sm">
                <div>
                ...
                <span className="text-xs bg-emerald-800 px-3 py-1.5 rounded-lg border border-emerald-700 text-emerald-100">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center">
                <div className="p-3 bg-emerald-50 rounded-xl border border-emerald-200">
                  ...
                  <span className="text-lg font-black text-emerald-950">{totalLiveBirdsHarvest.toLocaleString()}</span>
                  <span className="text-[10px] text-emerald-700 block">~96% survival rate</span>
                </div>

                <div className="p-3 bg-emerald-50 rounded-xl border border-emerald-200">
                  <span className="text-xs text-slate-500 font-medium block">Total Meat</span>
                  <span className="text-lg font-black text-emerald-950">{(totalLiveBirdsHarvest * 2.7).toLocaleString()} kg</span>
                  <span className="text-[10px] text-emerald-700 block">2.7kg avg dressed</span>
                </div>

                <div className="p-3 bg-emerald-50 rounded-xl border border-emerald-200">
                  <span className="text-xs text-slate-500 font-medium block">Total Feed</span>
                  <span className="text-lg font-black text-emerald-950">{broilerFeedBagsTotal}</span>
                  <span className="text-[10px] text-emerald-700 block">50kg Finisher Bags</span>
                </div>
          ...
          <div className="pt-2 border-t border-slate-100 flex flex-col sm:flex-row gap-3">
            <button
              ...
              className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 rounded-xl text-xs transition-colors shadow-sm flex items-center justify-center gap-2"
            >
