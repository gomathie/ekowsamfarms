...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="sticky top-0 z-40 bg-white shadow-sm font-sans border-b border-blue-100">
      {/* Top Banner Announcement */}
      <div className="bg-blue-900 text-blue-100 text-xs py-2 px-4 border-b border-blue-800">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-2">
          <div className="flex items-center gap-4 flex-wrap justify-center md:justify-start">
            <a href={`tel:${FARM_INFO.phones[0]}`} className="flex items-center gap-1.5 hover:text-blue-300 transition-colors">
              <Phone className="w-3.5 h-3.5 text-blue-400" />
              <span>{FARM_INFO.phones[0]}</span>
            </a>
            <span className="hidden sm:inline text-blue-700">|</span>
            <a href={`mailto:${FARM_INFO.emails[0]}`} className="hidden sm:flex items-center gap-1.5 hover:text-blue-300 transition-colors">
          ...
          <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
            ...
            <button
              ...
              className="flex items-center gap-1.5 bg-blue-700 hover:bg-blue-600 text-white px-2.5 py-0.5 rounded-full text-xs font-medium transition-colors shadow-xs"
            >
              <Sparkles className="w-3 h-3 text-amber-300" />
              <span>Ask AI Advisor</span>
            </button>
            <div className="flex items-center gap-1 text-blue-300 bg-blue-950/60 px-2 py-0.5 rounded text-[11px] border border-blue-800">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between">
        ...
        >
          <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-blue-600 to-green-700 flex items-center justify-center text-white shadow-md group-hover:scale-105 transition-transform">
            <Leaf className="w-6 h-6 text-blue-200" />
          </div>
          <div>
            <div className="flex items-center gap-1.5">
              <span className="text-xl font-black tracking-tight text-blue-950 font-serif">EKOW SAM</span>
              <span className="text-xl font-bold tracking-tight text-blue-600">FARMS</span>
            </div>
            <p className="text-[10px] uppercase font-bold tracking-widest text-blue-800/80 -mt-1">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="hidden sm:flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-semibold text-sm transition-all shadow-sm hover:shadow-md active:scale-95"
          >
          ...
          <button
            ...
            className="relative p-2.5 rounded-xl bg-slate-100 hover:bg-blue-50 text-slate-700 hover:text-blue-700 transition-colors border border-slate-200"
          >
            <ShoppingBag className="w-5 h-5" />
            {cartCount > 0 && (
              <span className="absolute -top-1.5 -right-1.5 bg-blue-600 text-white text-xs font-black w-5 h-5 rounded-full flex items-center justify-center shadow-sm animate-pulse">
                {cartCount}
              </span>
            )}
          </button>
      ...
      {/* Mobile Menu Drawer */}
      {mobileMenuOpen && (
        <div className="lg:hidden bg-white border-b border-blue-100 px-4 pt-2 pb-6 space-y-1 animate-in slide-in-from-top-2">
          <div className="p-3 bg-blue-50 rounded-xl mb-3 flex items-center justify-between border border-blue-100">
            ...
            <button
              onClick={openAIAssistant}
              className="bg-blue-600 text-white text-xs font-bold px-2.5 py-1 rounded-md shadow-xs flex items-center gap-1"
            >
              <Sparkles className="w-3 h-3 text-amber-300" />
            ...
            >
              ...
              {item.badge && (
                <span className={`text-xs px-2 py-0.5 rounded-full font-bold ${
                  currentPage === item.id ? 'bg-blue-800 text-blue-100' : 'bg-amber-100 text-amber-800'
                }`}>
                  {item.badge}
                </span>
              )}
          ...
          <div className="pt-3 flex flex-col gap-2">
            <button
              onClick={() => handleNavClick('store')}
              className="w-full bg-blue-600 text-white font-bold py-3 rounded-xl flex items-center justify-center gap-2 shadow-sm"
            >
              <ShoppingBag className="w-5 h-5" />
