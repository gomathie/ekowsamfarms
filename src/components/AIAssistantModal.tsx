      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full h-[600px] flex flex-col overflow-hidden border border-slate-100 relative">
        ...
        <div className="p-4 bg-brand-900 text-white flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-brand-700 flex items-center justify-center text-accent-300">
              <Sparkles className="w-5 h-5" />
            </div>
            <div>
              <h3 className="font-bold text-base leading-tight">Ekow Sam AI Farm Advisor</h3>
              <p className="text-xs text-brand-200">24/7 Agribusiness & Produce Assistant</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        {/* Quick Suggestions */}
        <div className="bg-brand-50 px-4 py-2 border-b border-brand-100 flex items-center gap-1.5 overflow-x-auto text-xs no-scrollbar">
          <HelpCircle className="w-3.5 h-3.5 text-brand-700 shrink-0" />
          {quickPrompts.map((prompt, idx) => (
            <button
              key={idx}
              onClick={() => handleSend(prompt)}
              className="whitespace-nowrap bg-white hover:bg-brand-100 text-brand-800 border border-brand-200 px-2.5 py-1 rounded-full text-[11px] font-medium transition-colors"
            >
              {prompt}
            </button>
          ))}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-slate-50/50">
          {messages.map((msg) => (
            ...
            >
              ...
              >
                {msg.sender === 'user' ? <User className="w-4 h-4" /> : <Bot className="w-4 h-4 text-accent-300" />}
              </div>
        ...
        >
          <input
            ...
            className="flex-1 text-xs px-3.5 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-brand-600 bg-slate-50"
          />
          <button
            type="submit"
            disabled={!input.trim() || loading}
            className="bg-brand-600 hover:bg-brand-700 disabled:opacity-50 text-white p-2.5 rounded-xl transition-colors shadow-xs"
          >
